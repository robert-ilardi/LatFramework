...
import static com.ilardi.systems.lat.LatConstants.PROPS_VALUE_VARIABLE_TOKEN_EXACT_MATCH;
import static com.ilardi.systems.lat.LatConstants.PROPS_VALUE_VARIABLE_TOKEN_MATCH_KEY_NAME;
...
public class ChatKey implements Serializable, Comparable<ChatKey> {
  ...
  public String toString() {
    return "ChatKey [chatName=" + chatName + ", orderIndex=" + orderIndex
        + ", displayName=" + displayName + ", toolTip=" + toolTip + ", visible="
        + visible + "]";
  }
  ...
  public boolean matchesEntityMatchingExpression(String entityName) {
    ...
    if (PROPS_VALUE_VARIABLE_TOKEN_MATCH_KEY_NAME
        .equalsIgnoreCase(entityMatchingExpression)) {
      matched = chatName.equals(entityName);
    }
    else if (entityMatchingExpression
        .startsWith(PROPS_VALUE_VARIABLE_TOKEN_EXACT_MATCH)) {
